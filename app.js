...
// Substitua pelas chaves do seu projeto "Gestor Valente - TESTE"
const SUPABASE_URL = 'https://aqxccienrpqhwdqzusnh.co'; 
...
// NÃO PRECISA MAIS DA SERVICE ROLE AQUI
// O RLS deve estar DESATIVADO no banco de dados de teste para isso funcionar.

const { createClient } = supabase;
let sb, user = null; // Removido sbService
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadInitialData() {
        if (!user) return;
        try {
            // MUDANÇA AQUI: Usamos 'sb' em vez de 'sbService'
            const { data: cidadaosData, error: cidadaosError } = await sb
                .from('cidadaos')
                .select('*')
    ...
    // O restante do código permanece IDÊNTICO, pois já usava 'sb'
